...
| `databricks/bus-mapping-pipeline/bus_mapping_pipeline.py` | Separate, triggered DLT pipeline (`blf_bus_mapping`): infers which physical bus (Powertrain, Body, Chassis, ...) each logged CAN channel is by comparing message IDs observed per (`_source_file`, channel) in `blf_silver_can` against message IDs expected per bus (from `ecu_bus.csv` joined with `message_sender.csv`), scoring every (`_source_file`, channel, bus) triple with a recall-weighted F-beta score (`blf_bus_mapping_scores`) and solving the one-to-one assignment via the Hungarian algorithm in an `applyInPandas` group per `_source_file` into `blf_bus_mapping`. Solved per file rather than corpus-wide: channel wiring is consistent within one log but not guaranteed to be across logs (different vehicles, harness revisions, renumbering). Also publishes `blf_gold_signals_bus`, `blf_gold_signals` left-joined with `blf_bus_mapping` on (`_source_file`, channel) (read as an external batch table, not added onto `blf_gold_signals` itself, to avoid a cross-pipeline cycle back into `blf_ingestion`). Triggered, not continuous: the assignment needs every channel's observed set within a file at once. Formerly the standalone `databricks/infer-bus-mapping/infer_bus_mapping.py` script |
...
The separate `blf_bus_mapping` pipeline (also triggered) reads `blf_silver_can` as a plain batch Delta table — it needs no wheel and shares no lineage with `blf_ingestion`. It compares, per (`_source_file`, channel), the CAN IDs actually observed against each bus's expected CAN IDs (`blf.bus_mapping_ecu_bus_path` joined with `blf.bus_mapping_message_sender_path`), then solves the channel-to-bus assignment as a maximum-weight one-to-one match independently for each `_source_file` — channel wiring is consistent within one log but not guaranteed to be across logs. Run it after `blf_ingestion` has caught up. `blf_bus_mapping.note` flags a (file, channel) whose best score falls below `blf.bus_mapping_min_score` for manual review.
