...
`assets/ecu_bus.csv` / `assets/message_sender.csv` are what `databricks/infer-bus-mapping/infer_bus_mapping.py` (the original standalone CLI script, kept for offline single-log analysis outside Databricks) used as its `--ecu-bus` / `--message-sender` sample inputs.
