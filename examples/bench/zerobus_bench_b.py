"""Bench B of a two-bench pair whose only link to each other is Zerobus. Standalone:
run it directly, no main.py and no topology registry.

    [bench A -- zerobus_bench_a.py]                      [bench B -- this script]
...
Run with (from the repo root), alongside bench A and with the same RUN_ID:
    uv run --group testing python examples/bench/zerobus_bench_b.py --run-id "$RUN_ID" --duration 120
"""
...
# B -> A carries this bench's generator output; A -> B carries the other bench's.
...
def build(config: ConnectionConfig, run_id: str) -> TestBench:
    ...
    other bench owns -- bench A with the two table names swapped.
    """
    bench = TestBench(run_id=run_id)

    tx_bus = bench.add_bus(ZerobusDuplex(_bus_config(config, TABLE_B2A), name=f"{TABLE_B2A}_tx"))
    rx_bus = bench.add_bus(ZerobusDuplex(_bus_config(config, TABLE_A2B), name=f"{TABLE_A2B}_rx"))
