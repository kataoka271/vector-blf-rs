...
use super::message::{Can, CanFd, CanFd64, Dir, Ethernet, EthernetEx, Vlan};
...
impl<W: Write> Encoder<W> for Can {
    fn encode(&self, mut w: W) -> ParseResult<()> {
        ...
        let flags = (self.dir.to_u8() & 0x3) | if self.rtr { 0x80 } else { 0 };
        let mut data = [0u8; 8];
...
impl<W: Write> Encoder<W> for CanFd64 {
    fn encode(&self, mut w: W) -> ParseResult<()> {
        ...
        let flags = ((self.dir.to_u8() as u32) & 0x3) << 6
...
impl<R: Read> Decoder<R> for CanFd64 {
    ...
    fn decode(mut r: R) -> ParseResult<Self::Item> {
        ...
        Ok(CanFd64 {
            ...
            id: can_id,
            is_ext_id,
            dir: Dir::from_u8(((flags >> 6) & 0x3) as u8),
            rtr: flags & 0x0010 != 0,
            fdf: flags & 0x1000 != 0,
            brs: flags & 0x2000 != 0,
            esi: flags & 0x4000 != 0,
...
impl<W: Write> Encoder<W> for Ethernet {
    fn encode(&self, mut w: W) -> ParseResult<()> {
        let flags = self.dir.to_u8() as u16 & 0x3;
...
impl<R: Read> Decoder<R> for Ethernet {
    ...
    fn decode(mut r: R) -> ParseResult<Self::Item> {
        ...
        Ok(Ethernet {
            channel,
            dir: Dir::from_u8((flags & 0x3) as u8),
            src_addr,
            dst_addr,
            vlan,
            ether_type,
...
impl<W: Write> Encoder<W> for EthernetEx {
    fn encode(&self, mut w: W) -> ParseResult<()> {
        ...
        (self.dir.to_u8() as u16).encode(&mut w)?;
...
impl<R: Read> Decoder<R> for EthernetEx {
    ...
    fn decode(mut r: R) -> ParseResult<Self::Item> {
        ...
        let mut p = &frame[..];
        let (dst_addr, src_addr, vlan, ether_type, data) = if frame.len() >= 14 {
            let mut dst_addr = [0u8; 6];
            ...
            ([0u8; 6], [0u8; 6], None, 0u16, frame)
        };
        Ok(EthernetEx {
            channel,
            dir: Dir::from_u8((dir & 0x3) as u8),
            src_addr,
            dst_addr,
            vlan,
            ether_type,
